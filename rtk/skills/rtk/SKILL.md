...
name: rtk
description: >
  RTK (Rust Token Killer) usage rules. Always prefix terminal commands with
  rtk to reduce token consumption by 60-99%. RTK is a transparent proxy — if
  it has a dedicated filter it uses it; if not, it passes through unchanged.
  Always safe to use.
---
