...
<div class="min-h-screen bg-background text-foreground flex">
	<!-- Sidebar desktop -->
	<aside
		class="hidden md:flex border-r border-border bg-slate-900 flex-col transition-all duration-200"
	...
	{#if mobileOpen}
		...
		<aside class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 border-r border-border flex flex-col md:hidden">
	...
	<div class="flex-1 flex flex-col min-w-0">
		<header class="h-14 border-b border-border bg-background/95 backdrop-blur flex items-center px-4 gap-4">
			<button class="md:hidden text-muted-foreground hover:text-foreground" onclick={() => (mobileOpen = true)} aria-label="Abrir menu">
			...
			<button
				class="hidden md:flex items-center gap-2 rounded-md border border-border bg-background px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground"
				onclick={openCommand}
			>
				<span>Buscar...</span>
				<kbd class="rounded bg-muted px-1.5 text-xs">Ctrl K</kbd>
			</button>
		</header>
...
{#if commandOpen}
	<div class="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/50" onclick={closeCommand} role="presentation">
		<div class="w-full max-w-lg rounded-xl border border-border bg-slate-900 shadow-2xl overflow-hidden" onclick={(e) => e.stopPropagation()} role="dialog" aria-label="Command palette">
